...
class PersonControllerTests extends OWFGroovyTestCase {
    ...
    void testListPersons() {
      ...
      Person.build(username: 'testUser1'.toUpperCase())
    ...
    void XtestListActionWithMaxParam() {
		Person.build(username:'person1'.toUpperCase())
		Person.build(username:'person2'.toUpperCase())
		Person.build(username:'person3'.toUpperCase())
		Person.build(username:'person4'.toUpperCase())
		Person.build(username:'person5'.toUpperCase())
	...
	void testShowAction() {
		...
		def person = Person.build(username:'person1'.toUpperCase(), authorities:authorities)


		controller.params.id = person.id
		def model = controller.show()

		assertEquals 'person1'.toUpperCase(), model.person.username
	...
	void testDeleteAction() {
		loginAsUsernameAndRole('testAdmin1', ERoleAuthority.ROLE_ADMIN.strVal)
		def person = Person.build(username:'testAdmin2'.toUpperCase())

		assertEquals 'testAdmin2'.toUpperCase(), Person.get(person.id).username
