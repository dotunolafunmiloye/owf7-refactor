...
class WidgetDefinitionServiceTests extends GrailsUnitTestCase {
	...
	protected void setUp() {
		...
		widgetDefinitionService.pWDService = personWidgetDefinitionServiceControl.createMock()
		...
		marketplaceServiceControl.demand.addListingsToDatabase(0..9999) { setOfJsonStrings ->
			private List<WidgetDefinition> updatedWidgets = setOfJsonStrings.collect { obj ->
		...
		PersonWidgetDefinition.metaClass.mockTags = []
		PersonWidgetDefinition.metaClass.getTags = {
			return delegate.mockTags.collect {[tag:[name:it]]}
		}
		PersonWidgetDefinition.metaClass.removeTag = {name -> delegate.mockTags.remove(name)}
		PersonWidgetDefinition.metaClass.addTag = {name, visible, position, editable -> delegate.mockTags << name}
	}
