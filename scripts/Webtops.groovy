...
target(cleanTomcat: "Scrubs the Tomcat directory, leaving only the WAR file") {
	def warname = System.getProperty('APP_CONTEXT') ?: "owf"

	tomcatDirs.each { tomcatDir ->
		println "| Webtops: clean out Tomcat directory ${tomcatDir}"
		delete(dir: "${tomcatDir}/logs")
		delete(dir: "${tomcatDir}/temp")
		delete(dir: "${tomcatDir}/work")
		delete(includeemptydirs: true) {
			fileset(dir: "${tomcatDir}/webapps") {
				include(name: '**/*')
			}
		}
		...
		println "| Webtops: copy WAR to Tomcat directory ${tomcatDir}"
		copy(tofile: "${tomcatDir}/webapps/${warname}.war", file: "${basedir}/target/${grailsAppName}-${metadata.getApplicationVersion()}.war", overwrite: true)
		println "| Webtops: copy OwfConfig to Tomcat directory ${tomcatDir}"
