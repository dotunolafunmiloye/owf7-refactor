Ext.define('Ozone.components.pane.Pane', 
{
	...
	plugins : [ new Ozone.plugins.pane.Pane() ],
	...
	// to be overriden in each pane that extends this class can be used to
	// restore state
	...
	initComponent : function() {
		var me = this, widgetStore = Ext.StoreManager.lookup('widgetStore'), widgetStates;
		...
		// clearing widgets array check to see if the user still has access to
		// the widget sync Widget Definition model properties with Widget State
		// model here
		me.widgets = [];
		...
		me.addEvents(OWF.Events.Widget.BEFORE_LAUNCH,
				OWF.Events.Widget.AFTER_LAUNCH);
		me.enableBubble([ OWF.Events.Widget.BEFORE_LAUNCH,
				OWF.Events.Widget.AFTER_LAUNCH ]);
	...
	getWidgets : function() {
		var widgets = Ext.clone(this.items.items), stateStore = this.stateStore;
	...
	getStatesOfAllWidgets : function() {
		var state = [], widgets = this.getWidgets();
	...
	launchFloatingWidget : function(model, x, y, instanceId, launchData,
			isDesktop) {
		var me = this, widgetCfg, widget;
		...
		if (!model.data.uniqueId) {
			if (me.defaultSettings && me.defaultSettings.widgetStates
					&& me.defaultSettings.widgetStates[widgetCfg.widgetGuid]) {
		...
		// must do outside of merge to prevent from getting cloned into
		// Ext.util.Animate object must render floating windows to pane's body
		// el to be able to shim each pane and get mousemove events be fired on
		// pane instead of iframe if mouse is moved to fast
		widgetCfg.renderTo = isDesktop ? me.body : me.dashboard.body;

		// show widget
		widget = Ext.widget('widgetwindow', widgetCfg);

		// register with the floating widget zindexmanager so always on top of
		// other widgets
		!isDesktop
				&& this.dashboard.dashboardContainer.floatingWidgetManager
						.register(widget);
		...
		if (isDesktop) {
			...
			// register with the top zindexmanager so always on top of other
			// widgets
			this.dashboard.dashboardContainer.floatingWidgetManager
					.register(widget);
	...
	// is called after dashboard layout is done launches previously opened
	// widgets
	...
	// launches an array of widgets used when a drag and drop is performed from
	// widget launch menu
	launchWidgets : function(models, x, y, launchPosition) {
		var widgetModel, deferreds = [];
		if (!models)
			return;
		...
		for ( var i = 0, len = models.length; i < len; i++) {
			...
			widgetModel = widgetModel.isModel ? widgetModel : Ext.create(
					'Ozone.data.State', widgetModel);

			if (this.fireEvent(OWF.Events.Widget.BEFORE_LAUNCH, this,
					widgetModel) !== false) {
				if (widgetModel.get('floatingWidget')) {
					deferreds.push(this.launchFloatingWidget(widgetModel, x, y,
							null, null, launchPosition));
				} else {
					deferreds.push(this.launchWidget(widgetModel, x, y, null,
							null, launchPosition));
				}
	...
	createWidgetConfig : function(model, instanceId, launchData) {
		...
		widgetCfg = {
			...
			model : model, // can be widget defination or state model (page
			// refresh)
			pane : me,
	...
	cloneWidgetDataForTransfer : function(widget) {
		var data = Ext.clone(widget.model.data), widgetDefinitionModel = this.dashboard.widgetStore
				.findRecord('widgetGuid', widget.model.get('widgetGuid'));
	...
	reorderWidget : function(widget, newPosition, oldPosition) {
		var me = this, widgetStateModelData = me
	...
	onBeforeClose : function(obj) {
		if (this.dashboard.configRecord.get('locked') && !obj.floatingWidget) {
			Ozone.Msg.alert(
					Ozone.layout.DialogMessages.dashboardLockAlertTitle,
					Ozone.layout.DialogMessages.dashboardLockAlert, null, null,
					null, this.dashboard.dashboardContainer.modalWindowManager);
			return false;
		}
		return true;
	},
	...
	onWidgetRender : function() {
		...
		this.pane.fireEvent(OWF.Events.Widget.AFTER_LAUNCH, this, this.model,
				false);

		this.add({
			xtype : 'widgetiframe',
			iframeProperties : OWF.Container.Eventing.getIframeProperties(
					this.model.get('url'), this.id, this.model.data,
					this.pane.type, this.launchData,
					this.dashboard.configRecord.get('locked'))
		});
	...
	disableDrop : function() {
		this.shimEl
				&& this.shimEl.removeCls('highlight-dashboard-designer-drop');
	},
