...
Ext.define('Ozone.components.button.UserMenuButton',
{
	...
	initComponent : function() {
				...
				handler : Ext.bind(
					function() {
						if (this.profileWindow == null
								|| this.profileWindow.isDestroyed) {
							this.profileWindow = Ext
									.widget(
											'profileWindow',
											{
	    ...
	    Ozone.KeyMap.addBinding([ Ext.apply({
			scope : this,
			fn : logout
		}, Ozone.components.keys.HotKeys.LOGOUT) ]);

		me.userMenu = new Ext.XTemplate('<ul id="userMenu">',
				'<tpl for=".">', '<tpl if="spacer">', '<hr/>',
				'</tpl>', '<tpl if="!spacer">',
				'<li id="{id}" ', '<tpl if="clickable">',
				'class="clickable"', '</tpl>', '><span>',
				'{text}', '</span>', '</li>', '</tpl>',
				'</tpl>', '</ul>');
	...
	getClickables : (function() {
		...
		return function() {
			if (!cached)
				cached = this.userMenu.el.select('.clickable');
			return cached;
		}
	})(),

    // automatically transition focus from the button to the menu and back
	setupFocusBridge : function() {
		// using apply to make a copy, since this is the flyweight object
		var firstMenuEl = Ext.apply({}, this.getClickables().first()), btnEl = this.getFocusEl();
    ...
    setupMenuNav : function() {
		...
		// given the dom node of one of the clickable menu items, focus either the next clickable 
		// one or the previous, depending on whether or not backwards is true
		function focusClickableSibling(dom, backwards) {
			var newNode = Ext.get(dom), property = backwards ? 'previousSibling' : 'nextSibling';

			// look backwards or forwards through the nodes until another clickable one is found
    ...
    addMenuShowHideHandlers : function() {
		var me = this, menuEl = me.userMenu.el, clickables = me.getClickables();
    ...
    showUserMenu : function() {
		var me = this, menuEl = me.userMenu.el;
		me.hideMenu = false;

		if (!me.showTask) {
			me.showTask = Ext.create('Ext.util.DelayedTask',
				function() {
					me._showMenu();
				});
		}
    ...
    hideUserMenu : function() {
		...
		if (!me.hideTask) {
			me.hideTask = Ext.create('Ext.util.DelayedTask',
				function() {
					me._hideMenu();
				});
		}
    ...
    _showMenu : function() {
		...
		/*
		 * if it is currently fading out, stop it so that
		 * setVisible(false) does not get called out of order
		 */
