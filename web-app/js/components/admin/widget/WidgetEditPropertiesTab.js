Ext.define('Ozone.components.admin.widget.WidgetEditPropertiesTab', 
{
	...
	alias : [ 'widget.widgeteditproperties', 'widget.widgeteditpropertiestab',
			...
			items : [
					{
						...
						listeners : {
							afterrender : function(cmp) {
								// After rendering the component will have an iconEl
								// property
								cmp.iconEl.on('click', function() {
									var descriptorUrlInfoMsg = cmp.ownerCt
											.getComponent('descriptorUrlInfoMsg');
								...
								Ozone.components.focusable.Focusable.setupFocus(
										cmp.iconEl, this);
								// pressing enter on the header toggles collapse
								new Ext.util.KeyMap(cmp.iconEl, {
									key : [ Ext.EventObject.ENTER ],
									fn : function(key, evt) {
										// required for IE, focus goes back to active
										// widget for some reason
										evt.stopEvent();

										var descriptorUrlInfoMsg = this.ownerCt
												.getComponent('descriptorUrlInfoMsg');
										if (descriptorUrlInfoMsg.isVisible()) {
											descriptorUrlInfoMsg.hide();
										} else {
					...
					},
					{
						...
						listeners : {
							...
							specialkey : {
								fn : function(field, e) {
									// e.HOME, e.END, e.PAGE_UP,
									// e.PAGE_DOWN, e.TAB, e.ESC
									// arrow keys:
						...
						items : [
								{
									...
									handler : function(btn) {
										...
										this.getComponent('descriptorUrlErrorMsg')
												.hide();
									},
									scope : this
								},
								'->',
								{
									...
									handler : function(btn) {
										...
										if (btnText == 'Load') {
											...
											var loading = this
													.getComponent('descriptorUrlLoading');
											var field = this
													.getComponent('descriptorUrl');
											if (this.xhr) {
												this.xhr.cancel();
											}
											loading.hide();
					...
					},
					{
						...
						usePlaceholderIfAvailable : false,
						fieldLabel : Ozone.util
								.createRequiredLabel('Container Icon URL')
					},
					{
						xtype : 'urlfield',
						name : 'image',
						...
						usePlaceholderIfAvailable : false,
						fieldLabel : Ozone.util
								.createRequiredLabel('Launch Menu Icon URL')
					},
					{
						xtype : 'numberfield',
						name : 'width',
		...
		this.on('afterrender', function() {
			...
			if (!widgetType.value) {
				this.widgetTypeStore.load({
					callback : function() {
						widgetType.setRawValue(widgetType.store.findRecord(
	...
	initFieldValues : function(record) {
		...
		if (data) {
			var strTags = "";
			var newGuid = (data && data.widgetGuid) ? data.widgetGuid
					: guid.util.guid();
			var widgetGuid = component.getComponent('widgetGuid'), name = component
					.getComponent('name'), description = component
					.getComponent('description'), version = component
					.getComponent('version'), universalName = component
					.getComponent('universalName'), guid = component
					.getComponent('guid'), url = component.getComponent('url'), headerIcon = component
					.getComponent('headerIcon'), image = component
					.getComponent('image'), width = component
					.getComponent('width'), height = component
					.getComponent('height'), tags = component
					.getComponent('_tags'), singleton = component
					.getComponent('singleton'), visible = component
					.getComponent('visible'), background = component
					.getComponent('background'), widgetType = component
					.getComponent('_types'), descriptorUrl = component
					.getComponent('descriptorUrl');
			...
			// Update the descriptorUrlInfo title and help message for editing
			// an existing widget
			component.getComponent('descriptorUrlInfo').titleEl.dom.innerHTML = 'Update Widget from Descriptor URL';
			component
					.getComponent('descriptorUrlInfoMsg')
					.update(
							'Click Load to update the widget. If the widget descriptor file changed since it was added to your instance of OWF, clicking Load will retrieve the latest widget data. To upload it to your OWF, click Apply.');
	...
	loadDescriptor : function(component) {
		var field = component.getComponent('descriptorUrl'), loading = component
				.getComponent('descriptorUrlLoading'), text = Ext.String
				.trim(field.getValue()), btn = component.getComponent(
				'descriptorUrlToolbar').getComponent('descriptorUrlBtn'), applyBtn = this
				.getDockedItems('toolbar[dock="bottom"]')[0]
				.getComponent('apply'), manualEntryLinkBtn = descriptorUrlToolbar
				.getComponent('manualEntryLinkBtn');
		...
		component.xhr = Ozone.util.Transport.send({
			...
			method : "GET",
			forceXdomain : true,
			onSuccess : Ext.bind(component.updatePropertiesFromDescriptor,
					component),
	...
	updatePropertiesFromDescriptor : function(data) {
		...
		loading.hide();
		var btn = this.getComponent('descriptorUrlToolbar').getComponent(
				'descriptorUrlBtn');
		btn.setText('Load');
		...
		if (data) {
			var strTags = "";
			var newGuid = (data && data.widgetGuid) ? data.widgetGuid : this
					.generateNewGuid();
			var widgetGuid = component.getComponent('widgetGuid'), name = component
					.getComponent('name'), description = component
					.getComponent('description'), version = component
			...
			for ( var i = 0; data && data.defaultTags
					&& i < data.defaultTags.length; i++) {
			...
			if (!Ext.isNumeric(typeId)) {
				widgetType.setRawValue(widgetType.store.findRecord('name',
						'standard').get('name'));
				typeId = this.widgetTypeStore.getAt(
						this.widgetTypeStore.find('name', typeId)).get('id');
			}
	...
	showProperties : function(show) {
		...
		var manualEntryLinkBtn = descriptorUrlToolbar
				.getComponent('manualEntryLinkBtn');
		manualEntryLinkBtn.hide();

		var component = this;
		var horizontalRule = component.getComponent('horizontalRule'), manualEntryTitle = component
				.getComponent('manualEntryTitle'), name = component
				.getComponent('name'), description = component
				.getComponent('description'), version = component
				.getComponent('version'), universalName = component
				.getComponent('universalName'), guid = component
				.getComponent('guid'), url = component.getComponent('url'), headerIcon = component
				.getComponent('headerIcon'), image = component
				.getComponent('image'), width = component.getComponent('width'), height = component
				.getComponent('height'), tags = component.getComponent('_tags'), singleton = component
				.getComponent('singleton'), visible = component
				.getComponent('visible'), background = component
				.getComponent('background'), widgetType = component
				.getComponent('_types');
	...
	handleDescriptorUrlChange : function(field, newValue, oldValue, eOpts) {
		...
		descriptorUrlBtn = descriptorUrlToolbar
				.getComponent('descriptorUrlBtn');
		if (!field.changed && field.isDirty())
			field.changed = true;
	...
	onApply : function() {
		...
		if (!this.getForm().hasInvalidField()) {
			...
			// Collect form values. Can't use this.getValues() because if
			// descriptorUrl is used fields may be disabled.
			var formFields = [ 'widgetGuid', 'name', 'description', 'version',
					'universalName', 'guid', 'url', 'headerIcon', 'image',
					'width', 'height', '_tags', 'singleton', 'visible',
					'background', '_types' ];
			// var formFields = ['widgetGuid', 'name', 'description', 'version',
			// 'universalName', 'guid', 'url', 'headerIcon', 'image', 'width',
			...
			for ( var i = 0; i < formFields.length; i++) {
				...
				if (Ext.isEmpty(value)
						|| (cmp.inputEl && cmp.inputEl.hasCls(cmp.emptyCls))) {
			...
			// If descriptor url was successfully loaded save it, otherwise
			// ignore it so we don't change the record's previous value.
				...
				for ( var j = 0; j < splits.length; j++) {
					...
					if (name != '') {
						tags.push({
							...
							// todo use position to order groups for now just
							// set to -1
							position : -1,
							editable : true
						});
			...
			this
					.showApplyAlert('Invalid field, changes cannot be saved.',
							3000);
		}
	},
