Ext.define('Ozone.components.dashboard.DashboardContainer', 
	...
	alias : [ 'widget.dashboardContainer',
			'widget.Ozone.components.dashboard.DashboardContainer' ],
	...
	// the number of milliseconds to wait between checks to determine if a save
	// is needed; 900000 = 15 minutes
	pollingInterval : Ozone.config.autoSaveInterval ? Ozone.config.autoSaveInterval
			: 900000,
	...
	initComponent : function() {
		...
		OWF.Container.Launcher = new Ozone.launcher.WidgetLauncherContainer(
				OWF.Container.Eventing);
		this.addWidgetContainer = new Ozone.marketplace.AddWidgetContainer(
				OWF.Container.Eventing);
		OWF.Container.DragAndDrop = new Ozone.dragAndDrop.WidgetDragAndDropContainer(
				{
					eventingContainer : OWF.Container.Eventing
				});
		OWF.Container.DragAndDrop.addCallback('dragStart', Ext.bind(function(
				data) {
			Ext.getCmp(this.activeDashboard.id).fireEvent('widgetStartDrag',
					data);
		}, this));
		OWF.Container.DragAndDrop.addCallback('dragStop', Ext.bind(function(
				data) {
			Ext.getCmp(this.activeDashboard.id).fireEvent('widgetStopDrag',
					data);
		}, this));
		...
		OWF.Container.State = new Ozone.state.WidgetStateContainer(
				OWF.Container.Eventing);
		...
		Ext.apply(this, {
			...
			items : [
			// putting the banner as an actual child component because there are
			// window contraining issues if the banner is an actual docked
			// toolbar
			{
				itemId : 'owfbanner',
				xtype : 'owfbanner',
		...
		this.on('resize', function(comp, width, height, oldWidth, oldHeight,
				eOpts) {
			var banner = this.getBanner();
			if (banner && banner.popOutToolbar
					&& banner.popOutToolbar.isVisible()) {
		...
		this.onBeforeUnload = function(evt) {
			...
			// tell History to shutdown cleanly because the browser is leaving
			// OWFs
		...
		// delegate all mousedown to document enables moving widgets from one
		// pane to another
	...
	onWidgetMouseDown : function(evt, target) {
		...
		var sourcePaneWidgets = this._sourcePane.getWidgets(), body = Ext
				.getBody();
		...
		for ( var pos = 0, len = sourcePaneWidgets.length; pos < len; pos++) {
			...
			widgetBox.midpoint = widget.tab ? widgetBox.x
					+ (widgetBox.width / 2) : widgetBox.y
					+ (widgetBox.height / 2);
	...
	onMouseMove : function(evt, target) {
		var xy = evt.getXY(), targetPane = Ext.get(target).up('.pane'), isFloating = this._widgetToMove
				.isFloating();
		...
		if (this._counter) {
			...
			if (this.activeDashboard.configRecord.get("locked")) {
				...
				this.ddProxy.setStatus(targetPane ? this.ddProxy.dropAllowed
						: this.ddProxy.dropNotAllowed);
			}
			...
			if (targetPane && this._sourcePane.el === targetPane) {
				if (isFloating) {
					...
					var match = false, eventPageX = xy[0], eventPageY = xy[1];
					...
					for ( var pos = 0, len = this._paneWidgets.length; pos < len; pos++) {
						...
						if (overWidget.tab) {
							if (widgetBox.x < eventPageX
									&& ((widgetBox.x + widgetBox.width) > eventPageX)) {
								match = true;
								break;
							}
						} else {
							if (widgetBox.y < eventPageY
									&& ((widgetBox.y + widgetBox.height) > eventPageY)) {
								match = true;
								break;
							}
						}
					...
					if (this._lastDropIndicatedWidget) {
						this._lastDropIndicatedWidget.el.removeCls
								.call(
										this._lastDropIndicatedWidget.tab ? this._lastDropIndicatedWidget.tab.el
												: this._lastDropIndicatedWidget.el,
										[ 'indicate-drop-before',
												'indicate-drop-after' ]);
					}

					if (pos === this._widgetToMove._position) {
						// don't move as the widget hasn't been moved
						this.ddProxy.setStatus(this.ddProxy.dropNotAllowed);
					} else if (match) {
						...
						this._lastDropIndicatedWidget.el.addCls
								.call(
										this._lastDropIndicatedWidget.tab ? this._lastDropIndicatedWidget.tab.el
												: this._lastDropIndicatedWidget.el,
										clsToAdd);
			...
			this.ddProxy.update('<img src="'
					+ this._widgetToMove.model.get('image')
					+ '" class="widget-drag-proxy"/>');
	...
	onMouseUp : function(evt, target) {
		...
		if (this._movePosition
				&& this._widgetToMove._position < this._movePosition)
			this._movePosition -= 1;

		if (this._reorderingWidgets && this._movePosition !== undefined
				&& this._movePosition !== this._widgetToMove._position) {
			this._sourcePane.reorderWidget(this._widgetToMove,
					this._movePosition);
		} else if (this._widgetToMove
				&& (targetPaneEl = Ext.fly(target).up('.pane'))) {
			this.activeDashboard.moveWidgetToPane(evt, Ext
					.getCmp(this._widgetToMove.paneGuid), Ext
					.getCmp(targetPaneEl.dom.id), this._widgetToMove);
		}
	 ...
	 * 
	 * Returns: a promise object that will be resolved with event and pane
	 * selected
	 */
	selectPane : function(isUsingKeyboard) {
		var me = this, deferred = jQuery.Deferred(), panes = this.activeDashboard.panes, doc = Ext
				.getDoc();
	...
	/*
	 * Manages pane selection using keyboard. Resolves deferred when Enter key
	 * is pressed and rejects when Esc is pressed
	 */
	_selectPaneOnKeyDown : function(e, dom, eOpts) {
		var panes = this.activeDashboard.panes, key = e.getKey(), lastPaneIndex = panes.length - 1, nextPaneIndex;
	...
	_selectPaneOnClick : function(e, dom, eOpts) {
		var paneEl = Ext.get(dom).up('.pane'), deferred = eOpts.deferred;
	...
	initLoad : function() {
		...
		if (this.dashboardStore.getCount() > 0) {
			for ( var i = 0, len = this.dashboardStore.getCount(); i < len; i++) {
				...
				this.originalDashboardStore.add(Ext.JSON.decode(Ext.JSON
						.encode(dashRecord.data)));
		...
		if (this.activeDashboard == null) {
			...
			if (this.defaultDashboard != null) {
				...
				if (this.dashboards.length > 0) {
					...
					this.activeDashboard = this.createDashboardConfig(this
							.createEmptyDashboard('desktop', true));
					this.defaultDashboard = this.activeDashboard;
		...
		Ext.state.Manager
				.setProvider(Ext
						.create(
								...
								{
									store : this.activeDashboard ? this.activeDashboard.stateStore
											: null
								}));
		...
		var setupInitialDash = Ext.bind(function() {
			...
			this.activeDashboard = dashboardCardPanel
					.getComponent(this.activeDashboard.id);
	...
	addKeyBindings : function() {
		...
		Ozone.KeyMap.addBinding([
				...
				Ext.apply(
						Ozone.components.keys.HotKeys.MAXIMIZE_COLLAPSE_WIDGET,
						{
							scope : this,
	...
	showWidgetSwitcher : function() {
		var widgetSwitcherId = 'widget-switcher', widgetSwitcher = Ext
				.getCmp(widgetSwitcherId);
		...
		if (!widgetSwitcher) {
			widgetSwitcher = Ext.widget('widgetswitcher', {
				...
				plugins : new Ozone.components.keys.HotKeyComponent(
						Ozone.components.keys.HotKeys.WIDGET_SWITCHER)
			});
	...
	showDashboardSwitcher : function() {
		...
		var dashboardSwitcherId = 'dashboard-switcher', dashboardSwitcher = Ext
				.getCmp(dashboardSwitcherId);
		...
		if (!dashboardSwitcher) {
			dashboardSwitcher = Ext.widget('dashboardswitcher', {
				...
				plugins : new Ozone.components.keys.HotKeyComponent(
						Ozone.components.keys.HotKeys.DASHBOARD_SWITCHER)
			});
	...
	activateDashboard : function(guid, silent) {
		// set dashboard in history but disable events so we don't activate the
		// dashboard twice
	...
	// this function is private, do not call outside of this class - use
	// activateDashboard instead
		...
		if (guid != null && guid != '') {
			...
			if (!params.guid) {
				if (guid == '') {
					...
					// todo remove this defer when there are events on
					// dashboards
					Ext
							.defer(
									Ozone.Msg.alert,
									...
									this,
									[
											Ozone.util.ErrorMessageString.invalidDashboard,
											this.activeDashboard.isdefault ? Ozone.util.ErrorMessageString.invalidDashboardMsg
	...
	/*
	 * the escape key has dual functionality. From within a widget, it escapes
	 * the widget. From within the container, it blurs in order to reset the
	...
	showCreateWindow : function() {
		...
		if (!win) {
			var createNewDashboardWindowConfig = {
				...
				listeners : {
					show : function(cmp) {
						var newDashboardPanel = this
								.getComponent('createNewDashboardPanel');
						var titleBox = newDashboardPanel
								.getComponent('titleBox');
						titleBox.setRawValue('');

						var importFileupload = newDashboardPanel
								.down('#importFileupload');
						...
						if (newDashboardPanel.afterRenderInitComplete == false) {
							...
							var existViewCb = newDashboardPanel
									.down('#existViewCb');
							existViewCb.reset();
							existViewCb.disable();
	...
	initEventing : function() {
		Ozone.eventing.Container
				.init({
					// override getIframeId directly lookup iframe ids via ext
					// components
					getIframeId : {
					...
					// hook widgetEventingReady so loadTime may be captured per
					// widget
					widgetEventingReady : {
						fn : function(sender, message) {
							var initMessage = gadgets.json.parse(message);
							if (initMessage.loadTime != null) {
								var widgetCfg = gadgets.json
										.parse(initMessage.id);

								var loadTimeCfg = {
									id : widgetCfg.id,
									msg : 'Loaded in ' + initMessage.loadTime
											+ ' (ms)',
								...
								if (activeWidgets != null) {
									...
									var wrec = activeWidgets
											.getById(widgetCfg.id);
									if (wrec != null) {
										loadTimeCfg.name = wrec.get('name');
										loadTimeCfg.widgetGuid = wrec
												.get('widgetGuid');
										this.sendLoadTime(loadTimeCfg);
									} else {
										// widget is not in the store however it
										// will be so attach a listener on add
										activeWidgets
												.on({
													add : {
														fn : function(store,
															...
															if (w != null) {
																loadTimeCfg.name = w
																		.get('name');
																loadTimeCfg.widgetGuid = w
																		.get('widgetGuid');
																this
																		.sendLoadTime(loadTimeCfg);
															}
														},
														scope : this,
														single : true
					...
					onRoute : {
						fn : function(sender, subscriber, channel, message) {
							...
							// if the sender or subscriber is the container then
							// always let the message through
							if (sender != '..' && subscriber != '..') {
								// check if the sender and subscriber are on the
								// same dashboard if so allow the message else
								// deny it
								var senderObj = Ozone.util.parseJson(sender);
								var senderDashboard = this
										.findParentDashboard(senderObj.id);
								var subscriberObj = Ozone.util
										.parseJson(subscriber);
								var subscriberDashboard = this
										.findParentDashboard(subscriberObj.id);
								if (senderDashboard != null
										&& senderDashboard == subscriberDashboard) {
									returnValue = true;
								}
							} else {
								returnValue = true;
		...
		// if (intentConfig != null && intentConfig[owfdojo.toJson(intent)] !=
		// null) {
		// var intentConfigDest = intentConfig[owfdojo.toJson(intent)];
		//
		// //check if dest widgets exists on the dashboard, if they all don't
		// delete the data
		...
		// //just send the intent if destination(s) is specified via saved
		// intentConfig
		...
		// this.showLaunchMenuForIntents(intent, container, sender, data,
		// sendingCmp);
		// }
		// else {
	...
	// showLaunchMenuForIntents: function(intent, container, sender, data,
	// sendingCmp) {
	// var intentConfig = sendingCmp.intentConfig;
	//
	// //open launch menu
	...
	// }
	//
	// //dont load the launchMenu's widgetStore, this disables the searchPanel
	// from causing a load as well
	...
	// description: 'Widgets below are filtered against Intent: ' +
	// intent.action + ' ' + intent.dataType
	// });
	...
	// //now select
	// var separator = '!@//@!';
	// intentsTree.selectPath(separator + 'Root' + separator + intent.action +
	// separator + intent.dataType,
	// 'text', separator);
	//
	...
	// //this would only need to be done if a widget was opened that never
	// registered
	...
	// );
	// }
	// //fire callback to startActivity call once dest widgets have been
	// identified
	...
	//	                        
	// // remove the listener when finished because when launching new widgets
	// in this pane
	...
	// container.removeListener('onIntentsReady', deferredSendIntentListener,
	// this);
	// }
	...
	sendLoadTime : function(loadTimeCfg) {
		...
		if (Ozone.config.publishWidgetLoadTimes === true) {
			...
			Ozone.eventing.Container.publish('Ozone.eventing.widget.public',
					payload);
		}
	},
	...
	getOpenedWidgets : function() {
		var returnValue = [];
		if (this.activeDashboard != null
				&& this.activeDashboard.stateStore != null) {
	...
	deleteDashboards : function(dashboardsToDelete) {
		...
		for ( var i1 = 0; i1 < deleteCount; i1++) {
			...
			var dsIndex = this.dashboardStore.find('guid',
					dashboardsToDelete[i1]);
			if (dsIndex != -1) {
				// If default dashboard is deleted reset default to first
				// dashboard.
	...
	updateDashboardsFromStore : function(storeRecords, callbackOptions,
			loadSuccess, dashboardGuidToActivate) {
		// ---------------------------------------------------------------------------------
		...
		// Without the timeout, updateDashboardsFromStore causes problems when
		// dashboards are restored because widget destruction is delayed by
		// 100ms to prevent memory leaks. Because of the delay, widgets on a
		// dashboard get re-rendered while previous ones haven't been destroyed.
		setTimeout(function() {
			...
			for ( var i1 = 0, len = storeRecords.length; i1 < len; i1++) {
				...
				// Save default dashboard Guid and set active dashboard
				// instance.
			...
			// activate dashboard
			me._activateDashboard(dashboardGuidToActivate || defaultTabGuid);

			// Focus the default dashboard.
			me.activateDashboard(dashboardGuidToActivate || defaultTabGuid,
					true);
	...
	saveActiveDashboardToServer : function() {
		// only save if the active dashboard is in store it may have been
		// deleted
	...
	saveDashboard : function(json, createOrUpdate, success, failure) {
		...
		if (!failure) {
			failure = function() {
				Ozone.Msg.alert('Dashboard Manager',
						"Error creating or updating dashboard.", null, null,
						null, me.modalWindowManager);
				return;
			}
		}
	...
	editDashboard : function(model) {
		var dashboard = this.getDashboardCardPanel().getComponent(
				model.get('guid'));
	...
	designDashboard : function(dashboard, dashboardExists) {
		...
		var banner = this.getBanner().getBanner(), me = this;
					...
					callback : function(wrecords, woptions, wsuccess) {
						if (wrecords.length >= 0) {
							if (!wsuccess) {
								Ozone.Msg
										.alert(
												Ozone.util.ErrorMessageString.retrieveUpdatedWidgets,
												Ozone.util.ErrorMessageString.retrieveUpdatedWidgetsMsg,
												null, null, null,
												this.modalWindowManager);
							}
						} else {
	...
	closeWidget : function(key, event, widgetInstanceId) {
		var activeWidget = widgetInstanceId ? Ext.getCmp(widgetInstanceId)
				: this.activeDashboard.getActiveWidget();
	...
	saveWidgetState : function(key, event, widgetInstanceId) {
		var widget = widgetInstanceId ? Ext.getCmp(widgetInstanceId)
				: this.activeDashboard.getActiveWidget();
	...
	updateTitlesandBanner : function() {
		// Update widget titles with user-defined titles and find out if there
		// are marketplace widgets
		...
		// if we have a marketplace widget or marketplace config, tell the
		// banner to add a button
		if (hasMpWidget) {
