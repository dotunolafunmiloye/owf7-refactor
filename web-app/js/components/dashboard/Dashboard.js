Ext.define('Ozone.components.dashboard.Dashboard', 
{
	...
	plugins : [ new Ozone.plugins.Dashboard() ],
	...
	initComponent : function() {
		...
		this.addEvents('beforeserversave', 'serversave', 'widgetStartDrag',
				'widgetStopDrag', OWF.Events.Widget.BEFORE_LAUNCH,
				OWF.Events.Widget.AFTER_LAUNCH);

		this.enableBubble([ OWF.Events.Widget.BEFORE_LAUNCH,
				OWF.Events.Widget.AFTER_LAUNCH ]);
	...
	afterDashboardLayout : function() {
		var me = this, pane, allWidgetsDeferreds = [];
		...
		for ( var i = 0, len = this.panes.length; i < len; i++) {
			...
			allWidgetsDeferreds.push.apply(allWidgetsDeferreds, pane
					.launchPreviouslyOpenedWidgets());
		}
	...
	enableWidgetDragAndDrop : function() {
		this.dropZone = this.dropZone || new Ext.dd.DropZone(this.getEl(), {
			...
			// While over a target node, return the default drop allowed class
			// which places a "tick" icon into the drag proxy.
	...
	launchWidgets : function(target, dd, coordinates, data) {
		var targetPaneId = target.id, targetPaneEl, offsets, targetPane = Ext
				.getCmp(targetPaneId), x, y;
		...
		if (targetPane && targetPane.isXType('pane')) {
			...
			if (coordinates) {
				...
				targetPane.launchWidgets(data.widgetModel, x - offsets[0], y
						- offsets[1], 0);
			} else {
	...
	onBeforeWidgetLaunch : function(pane, widgetModel, launchData) {
		var stateStore = this.stateStore, instanceId = widgetModel
				.get('uniqueId')
				|| guid.util.guid(), isSingleton = widgetModel.get('singleton'), isBackground = widgetModel
				.get('background'), stateModel = stateStore.findRecord(
				'widgetGuid', widgetModel.get('widgetGuid')), isAlreadyLaunched = !!stateModel, widget;
		...
		if (isBackground) {
			widget = this.add({
				...
				iframeProperties : OWF.Container.Eventing.getIframeProperties(
						widgetModel.get('url'), instanceId, widgetModel.data,
						pane.id, launchData, this.configRecord.get('locked'))
			});
			...
			// workaround for now only way widgets get added to dashboard store
			// is if 'add' event fires on pane's store which it doesn't for
			// background widgets as they don't go through state changes so
			// remove and add so that same record gets added to both stores
		...
		if (!pane._launchingPreviouslyOpenedWidgets && !pane._reorderingWidgets
				&& pane.dashboard.configRecord.get('locked')) {
			pane.launchFloatingWidget(widgetModel, null, null, instanceId,
					launchData);
			return false;
		}
	...
	moveWidgetToPane : function(evt, sourcePane, targetPane, widget) {
		var me = this, widgetModel = widget.model, x = evt.getX(), y = evt
				.getY();

		if (!sourcePane || !targetPane || sourcePane === targetPane
				|| this.configRecord.get("locked"))
			return;

		// use widget defination model instead of using state model
		// so that it gets a new instance id
		widgetModel = this.widgetStore.findRecord('widgetGuid', widgetModel
				.get('widgetGuid'));

		function doIt() {
			if (targetPane.isXType('fitpane') && targetPane.items.length > 0) {
				targetPane.confirmReplaceWidget(widget.id, widget.title,
						function() {
		...
		function move() {
			var pane = targetPane, widgetStateModelData = pane
					.cloneWidgetDataForTransfer(widget);
	...
	/**
	 * @description Launches a widget instance based on the specified widget
	 *              guid.
	 * @member Ozone.layout.AccordionWindowManager
	 * @param JSON
	 *            object specifying the widget guid and an option
	 *            launchOnlyIfClosed param.
	 * @returns JSON object specifying success/failure, an error message, and
	 *          the uniqueId of the widget that was launched.
	 */
	launchWidgetInstance : function(sender, launchConfig) {
		var launchWidget = null, specifiedWidget = null, responseObj, pane = Ext
				.getCmp(sender.id).pane;
		...
		if (launchConfig.guid != null) {
			...
			var index = this.widgetStore.findExact('universalName',
					launchConfig.universalName);
			if (index > -1) {
		...
		if (specifiedWidget == null) {
			...
			// make a copy so we don't alter the widgetdef if we change the
			// title
			...
			if (launchConfig.title != null) {
				...
				if (launchConfig.titleRegex == null) {
					...
					var matches = launchConfig.titleRegex
							.match(/^\/(.*)\/(.*)$/);
					...
					if (matches != null && matches[1] != '') {
						...
						specifiedWidget.data.name = specifiedWidget.data.name
								.replace(new RegExp(matches[1], matches[2]),
										launchConfig.title);
					}
				}
			}
			...
			if (launchConfig.launchOnlyIfClosed) {
				...
				for ( var i = 0, len = this.stateStore.getCount(); i < len; i++) {
					...
					if (stateWidget != null
							&& stateWidget.get('widgetGuid') == launchConfig.guid) {
						...
						// Return failure, error message, and the uniqueId of
						// the widget instance
						responseObj = {
							error : false,
							newWidgetLaunched : false,
			...
			if (launchWidget == null) {
				...
				if (pane.fireEvent(OWF.Events.Widget.BEFORE_LAUNCH, pane,
						specifiedWidget, launchConfig.data) !== false) {
					pane.launchWidget(specifiedWidget, null, null, uniqueId,
							launchConfig.data, true);
				}
	 ...
	 * @param config
	 *            JSON object containing the function to call and the params
	...
	handleWidgetRequest : function(config) {
		...
		case 'addStateEventListeners':
			return this.addWidgetStateEvents('listen', config.params.guid,
					config.params.events);
		case 'addStateEventOverrides':
			return this.addWidgetStateEvents('override', config.params.guid,
					config.params.events);
		case 'removeStateEventListeners':
			return this.removeWidgetStateEvents('listen', config.params.guid,
					config.params.events);
		case 'removeStateEventOverrides':
			return this.removeWidgetStateEvents('override', config.params.guid,
					config.params.events);
		case 'closeWidget':
	 ...
	 * @member Ozone.components.dashboard.Dashboard
	 * @param eventAction
	 *            Action to take when an event is received. Valid values are
	 *            "listen" and "override".
	 * @param guid
	 *            Id of the widget
	 * @param events
	...
	addWidgetStateEvents : function(eventAction, guid, events) {
		// get widget by guid
		var widget = Ext.getCmp(guid), eventName;
		...
		if (widget) {
			...
			for ( var i = 0, len = events.length; i < len; i++) {
				...
				if (widget.tab && eventName.indexOf('close') > -1) {
					...
					widget["on_" + eventName] = stateEventHandler(widget,
	...
	removeWidgetStateEvents : function(eventAction, guid, events) {
		// get widget by guid
		var widget = Ext.getCmp(guid), eventName, cmp;
	...
	/**
	 * @description Activates a widget. This method should be overridden by each
	 *              dashboard that support activating a widget.
	 * @member Ozone.components.dashboard.Dashboard
	 * @param params
	 *            guid of the widget to activate.
	 */
	activateWidget : function(widgetInstanceId) {
		var widget = Ext.getCmp(widgetInstanceId);
		if (widget) {
			if (widget.floatingWidget) {
				widget.minimized === true ? widget.restoreFromMinimize(true)
	...
	/**
	 * @description Closes a widget. This method should be overridden by each
	 *              dashboard type.
	 * @member Ozone.components.dashboard.Dashboard
	 * @param params
	 *            guid of the widget to close.
	 */
	 ...
	 * @member Ozone.components.dashboard.Dashboard
	 * @param params
	 *            JSON object containing the guid of the widget to close.
	 */
	...
	/**
	 * @description Defines what to do when launchWidgetInstance method was
	 *              called on a widget that is already launched and
	 *              launchOnlyIfClosed is set to true. This method should be
	 *              overridden by each dashboard type.
	 * @member Ozone.components.dashboard.Dashboard
	 * @param widget
	 *            The widget instance in state
	 */
	handleAlreadyLaunchedWidget : function(widgetData) {
		var widgetCmp = Ext.getCmp(widgetData.uniqueId);

		this.fireEvent(OWF.Events.Widget.AFTER_LAUNCH, widgetCmp,
				widgetCmp.model, true);
	...
	getWidgetState : function(guid) {
		var widget = Ext.getCmp(guid), state, storeState, size;
	 ...
	 * @member Ozone.components.dashboard.Dashboard
	 * @param windowId
	 *            Id of window to be removed.
	 * @param dm
	 *            DashboardMgmt object.
	 * @returns None.
	 */
	...
	initNewWidget : function(widget) {
		...
		widget
				.on(
						"statesave",
						function() {
							// For each record in state, add/update value in
							// dashboard defaultStates object
	...
	onActivate : function(cmp) {
		...
		for ( var i = 0, len = items.length; i < len; i++) {
			var rec = items[i], uniqueId = rec.get('uniqueId'), cmp = Ext
					.getCmp(uniqueId);
			var onShow;
	...
	getJson : function() {
		var json = this.configRecord.data, pane, panes = this.query('pane'), paneWidgets;
	...
	saveToServer : function(sync, force, saveAsNew, successCallback,
			failureCallback) {
		// save state
	...
	getSite : function() {
		...
		if (matches && matches[1] && matches[1].length > 0) {
			...
			baseContextPath = baseContextPath.substring(0,
		...
		return window.location.protocol
				...
				+ (baseContextPath.charAt(0) != '/' ? ('/' + baseContextPath)
						: baseContextPath);
	},
	...
	updateActiveWidget : function(widget, notify) {
		...
		if (this.activeWidget && this.activeWidget !== widget) {
			...
			if (this.activeWidget.iframeReady && notify !== false) {
				...
				if (deactivatedWidgetId) {
					gadgets.rpc
							.call(deactivatedWidgetId, '_widget_deactivated');
				}
			}
		}
		...
		if (widget && notify !== false && this.activeWidget.iframeReady
				&& !widget._activated) {
			widget.active = true;
