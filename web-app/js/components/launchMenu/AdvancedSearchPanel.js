Ext.define('Ozone.components.launchMenu.AdvancedSearchPanel', 
{
		...
		var deleteColumn = {
	        ...
	        Ext.apply(this, {
	            ...
	            items: [
					...
					// plugins: [
					// new
					// Ozone.components.focusable.FocusableGridPanel()
					// ],
					// columns: [
					// {
					...
					// listeners: {
					// itemkeydown: {
					// fn: function(view, record, dom, index,
					// evt) {
					// switch(evt.getKey()) {
					...
					// case evt.SPACE:
					// case evt.ENTER:
					// view.selModel.selectWithEvent(record,
					// evt);
					//	                                            break;
					//	                                        default:
					//	                                            break;
	...
	// expanded: true,
	//
	// //todo see if we can fix this bug - tree nodes can still be expanded if
	...
	loadTagCloudStore : function() {
		...
		if (!this.dontLoadWidgetStore) {
			...
			if (overrideFilterCfg != null) {
				if (overrideFilterCfg.customWidgetName != null
						&& overrideFilterCfg.customWidgetName != '') {
					...
					termStore.add({
						term : overrideFilterCfg.customWidgetName
					});
				}
			...
			// loop through main widget store and add any widgets that meet the
			// filter
			...
			for ( var i = 0, len = containerWidgetStore.getCount(); i < len; i++) {
				...
				// check if visible
				keepWidgetFlag = widgetRec.get('visible')
						&& widgetRec.get('definitionVisible');
				...
				if (keepWidgetFlag) {
					for ( var j = 0; j < termStore.getCount(); j++) {
						var term = Ext.htmlDecode(termStore.getAt(j)
								.get('term'));
						isFiltered = true;

						if (widgetRec.get('name') != null
								&& !widgetRec.get('name').match(
										new RegExp('.*' + term + '.*', 'i'))) {
							keepWidgetFlag = false;
							break;
						}
				...
				// }
				// }
				// // no dataType so the intent matches just by
				// // action
				// else {
				// foundIntent = true;
				// }
