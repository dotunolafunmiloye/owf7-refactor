Ext.define('Ozone.components.launchMenu.LaunchMenu', 
{
	...
	initComponent : function() {
		...
		// make a copy of widgetStore to use, this is needed because filtering
		// causes problems when widget states are saved
		...
		// a load on the main widgetStore this will trigger the launch Menu
		// widgetStore to load
		...
		this.widgetStore.on({
			datachanged : {
				fn : function(store, eOpts) {
					...
					this.openedWidgetStore
							.filter({
								filterFn : function(rec, id) {
									var widgetId = rec.get('widgetGuid');
									return store.findExact('widgetGuid',
											widgetId) > -1;
								}
							});
				},
		...
		// create two favstores one to be filtered in the launch menu and the
		// other to be the full set
		...
		this.favStore.on({
			add : {
				fn : function(store, records, index, eOpts) {
					for ( var i = 0; i < records.length; i++) {
						...
						// make sure the record to be added is not already in
						// the favStoreUnfiltered
						if (this.favStoreUnFiltered.findExact('widgetGuid',
								record.get('widgetGuid')) < 0) {
							if (this.widgetStore.widgetFiltered) {
								this.favStoreUnFiltered.add(record);
							} else {
								this.favStoreUnFiltered.insert(index + i,
										record);
							}
						}
					}
			...
			remove : {
				fn : function(store, record, index, eOpts) {
					var favindex = this.favStoreUnFiltered.findExact(
		...
		var someObjectToApply = {
			...
			dockedItems : [ {
				...
				listeners : {
					afterlayout : {
						fn : function(cmp) {
							// ext toolbars auto subscribe to
							// the FocusManager in afterrender
				...
				items : [ {
					...
					toggleHandler : function(b, state) {
						...
						// focus the text box if this button was
						// pressed or clicked
						if (state) {
			...
			items : [
									...
									items : [
											{
												...
												flex : 1,
												layout : 'auto',
												// Ext hbox puts a 2 pixel gap
												// between things that we don't
												// want, so its easier if we
												// just lay it out in css items
												// are floated right, so they
					...
					},
					{
						...
						listeners : {
							...
							render : {
								fn : function(cmp) {
									var dropZone = new Ext.dd.DropZone(
											cmp.getEl(),
											{
												ddGroup : 'carousel',
		...
		Ext.apply(this, someObjectToApply);

		this.callParent();

		this.addEvents('noWidgetLaunched', 'refresh');

		me.carousel.on('render',
				function(cmp) {
					var v = cmp.down('#carouselView');

					// create a dropzone for the carousel view that allows
					// widgts to be saved as favs
					var dropZone = new Ext.dd.DropZone(cmp.getEl(), {
						...
						onNodeDrop : function(target, dd, e, data) {
							// figure out which side of the target widget the
							// new widget is to be added at
							var pt = v.getDropPoint(e, target, dd);

							// check if the origin view is the same as the
							// target view
							if (v == data.view) {
							...
							// check if the widget being dragged is in the dest
							// view's store
							var existingWidgetIndex = v.store.findExact(
									'widgetGuid', data.widgetModel
											.get('widgetGuid'));
							...
							// now determine where to insert the new widget
							// based on the target of the drag
							var targetIndex = v.indexOf(target);
							var record = v.store.getAt(targetIndex);

							// if the drop point is after the target widget(the
							// widget being hovered over) then increment the
							// index so the dropped widget appears after the
							// target widget
							if (pt == 'after') {
								targetIndex++;
							}
							...
							data.view.fireEvent('drag', v, record, target,
									targetIndex, e);
							return true;
						},
		...
		this.on({
			...
			beforeshow : {
				fn : function(cmp) {
					// adjust the size of the launch menu based on the size of
					// the browser
					this.setSize(document.body.clientWidth * 0.55,
							document.body.clientHeight * 0.7);

					// just to be sure un register the listener
					Ext.EventManager.un(window, 'beforeunload',
							this.saveLauncherStateSync, this);

					// register the beforeunload listener to save the launch
					// menu state while the launch menu is open
			...
			selectionchange : {
				fn : function(selModel, records, eOpts) {
					if (records.length > 0) {
						...
						this.updateInfoPanel(record, (eOpts && eOpts.favorite)
								|| this.favStore.findRecord("widgetGuid",
										record.get('widgetGuid')));
					}
				},
				scope : this
			},
		...
		me.on('afterrender', me.enableDraggingOut, me, {
			single : true
		});
	},

	refreshOpenedWidgets : function() {
		// if the activeDash is set then use the stateStore as the
		// openWidgetStore
		if (this.dashboardContainer.activeDashboard != null) {
			this.view
					.setOpenedWidgets(this.dashboardContainer.activeDashboard.stateStore);
		}
	},
	...
	enableDraggingOut : function() {
		...
		function move() {
			if (this.isDragging === true) {
				...
				// Ext has a bug where it executes ondrop events 2 times on just
				// one drop. This works around the bug by adding the same group
				// to active dashboard after drag starg and removes it on
				// drop.... Easier than patching Ext
				if (!srcDragZone.groups['widgets']) {
					isWidgetGroupAdded = true;
					srcDragZone.addToGroup('widgets');
				}
				this.dashboardContainer.activeDashboard
		...
		this.on('dragStarted', function(dragZone) {
			...
			if (!disableDragLaunching) {
				// this mousemove listener will only fire if the mouse is over
				// the .x-mask (mask put up for a modal window)
		...
		if (widgetDefs && widgetDefs.getCount() > 0) {
			...
			if (widgetDef.get('disabled') !== true) {
				if (widgetDef.get('singleton')) {
					...
					if (widget) {
						scope.activeDashboard
								.handleAlreadyLaunchedWidget(widget.data);
						ok2Add = false;
					}
				}
				// an already opened widget instance is being opened again
					...
					scope.activeDashboard
							.handleAlreadyLaunchedWidget(record.data);
					ok2Add = false;
				}
			} else {
				Ozone.Msg.alert(Ozone.util.ErrorMessageString.errorTitle,
						Ozone.util.ErrorMessageString.widgetNotApproved, null,
						null, null, scope.modalWindowManager);
				ok2Add = false;
			}

			if (ok2Add) {
				if (scope.activeDashboard.configRecord.get('locked')) {
					var instanceId = widgetDef.get('uniqueId')
							|| guid.util.guid();
					scope.activeDashboard.panes[0].launchFloatingWidget(
							widgetDef, null, null, instanceId);
				} else {
					// required to prevent from bubbling down
					evt.stopEvent();
					var newWidget = scope.launchWidgets(widgetDef, evt
							&& evt.getKey() === Ext.EventObject.ENTER);
				}
			}
		}
	 ...
	 */
	updateInfoPanel : function(record, showRemoveFavorites, disableLaunchButton) {
		// updateInfoPanel: function (record, showRemoveFavorites,
		// disableLaunchButton, showIntentCheckBox, intentCheckBoxValue) {
		var me = this, infoPanel = me.down("#infoPanel"), infoCenter = infoPanel
				.getComponent('infoCenter'), imagePanel = infoPanel
				.getComponent('imagePanel'), launchBtn = imagePanel
				.getComponent('launch'), htmlPanel = infoPanel
				.down('#htmlPanel'),
		// intentCheckBox = infoPanel.down('#intentCheckBox'),
		image = imagePanel.getComponent('widgetImg'), launchBtnHandler = function(
				button, evt) {
		...
		var desc = record.get('description') != null ? record
				.get('description') : '';
		// todo remove this hardcoded style
		var html = "<h1 class=\"launchMenuTitle\">"
				+ Ext.htmlEncode(record.get('name')) + "</h1>" + "<p>"
				+ Ext.htmlEncode(desc) + "</p>";
		image.setSrc(record.get('image') ? record.get('image')
				: 'images/launch-menu/launch-infoPanel-default.png');
	...
	itemClick : function(view, record, item, index, event, eOpts) {
		this.updateInfoPanel(record, (eOpts && eOpts.favorite)
				|| this.favStore.findRecord("widgetGuid", record
						.get('widgetGuid')));
	},
	...
	// decodes the given json into the view store and the favorites store guid
	// arrays
	loadFavStore : function(json) {
		...
		for ( var i = 0; i < favStoreIds.length; i++) {
			...
			var record = me.dashboardContainer.widgetStore.findRecord(
					"widgetGuid", widgetGuid);
			// only add if they exist in the main widgetStore and are visible
			// and aren't already in the favStore
			if (record && record.get('visible')
					&& !favStore.findRecord("widgetGuid", widgetGuid)) {
				favStore.add(record);
			}
		}
	...
	saveLauncherState : function(sync) {
		...
		if (!this.widgetStore.widgetFiltered) {
			...
			for ( var i = 0; i < records.length; i++) {
				...
				// check to see if the name was customized if so submit it to be
				// saved
	...
	createSidePanel : function() {
		this.searchPanel = Ext.widget('advancedsearchpanel', {
			...
			listeners : {
				beforeshow : {
					fn : function(cmp) {
						// adjust the size of the launch menu based on the size
						// of the browser
						cmp.setSize(document.body.clientWidth * 0.15,
								document.body.clientHeight * 0.65);
					},
					scope : this
				},
				show : {
	...
	openOrCloseAdvancedSearch : function(open, disableRefresh) {
		...
		if (open) {
			...
			if (searchbox.getValue() != '') {
				...
				// this will cause the widgetStore to reload and clear any
				// search
				if (!disableRefresh) {
					searchbox.onClear();
				}
	...
	setupCircularFocus : function() {
		...
		var focusCatchEl = Ext.DomHelper.append(this.el,
				'<div class="focus-catch" tabindex="0"></div>', true);
	...
	onClose : function() {
		...
		Ext.EventManager.un(window, 'beforeunload', this.saveLauncherStateSync,
				this);
	}
});
