Ext.define('Ozone.components.widget.WidgetWindow', 
{
	...
	alias : [ 'widget.widgetwindow',
			'widget.Ozone.components.widget.WidgetWindow' ],

	mixins : {
		widget : 'Ozone.components.widget.WidgetBase'
	},
	plugins : [ new Ozone.components.keys.KeyMoveable(),
	...
	onDeActivate : function(cmp) {
		if (this.enableDeactivateShim) {
			...
			if (!this.deactivateShim) {
				...
				this.deactivateShim.setZIndex(parseInt(this.el
						.getStyle('z-index')));
	...
	stateEvents : [ 'bodyresize', 'show', 'minimize', 'maximize', 'activate',
			'deactivate', 'titlechange' ],
	...
	getState : function() {
		...
		if (this.rendered) {
			...
			Ext.apply(o, {
				zIndex : Ext.isNumber(zIndex) && zIndex > 0 ? zIndex
	...
	toggleMaximize : function(event) {
		if (!event.getTarget('.' + this.baseCls + '-header-text')
				&& !event.getTarget('titleEditor')) {
	 ...
	 * the {@link #maximizable 'maximize' tool button} with the 'restore' tool
	 * button. Also see {@link #toggleMaximize}.
	 * 
	...
	minimize : function() {
		...
		if (!me.minimized) {
			...
			this.focusOnRestore = null; // on restore from minimize, focus the
			// widget body
	...
	restore : function() {
		...
		if (!this.tools.restore.isFocused())
			this.focusOnRestore = null;
