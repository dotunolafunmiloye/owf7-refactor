Ext.define('Ozone.components.widget.WidgetPanel', 
{
	...
	alias : [ 'widget.widgetpanel',
			'widget.Ozone.components.widget.WidgetPanel' ],
	mixins : {
		widget : 'Ozone.components.widget.WidgetBase'
	},
	plugins : [ new Ozone.plugins.WidgetPanel() ],
	dashboardGuid : null,
	...
	stateEvents : [ 'resize', 'collapse', 'expand', 'activate', 'deactivate',
			'minimize', 'maximize', 'titlechange' ],
	getState : function() {
		var o = {
	...
	afterCollapse : function(animated, internal) {
		this.callParent(arguments);
		// if this is an internal collapse we still need save state is keep the
		// collapsed property correct
		if (internal) {
			this.saveState();
		}
	}
