Ext.define('Ozone.components.widget.WidgetPortlet', 
{
	...
	/**
	 * @description Defines what state gets saved for the portal when the state
		...
		if (this.rendered) {
			Ext
					.apply(
							o,
							{
								height : this.collapseMemento != null
										&& this.collapseMemento.data != null ? this.collapseMemento.data.height
										: this.getSize().height,
	...
	/**
	 * @description Defines what state gets restored for the portal when the
	 *              state manager calls this method.
	 * 
	 * @param state
	 *            State data from by portal manager's state provider.
	 * @returns None.
	 */
	applyState : function(state) {
		...
		// only set collapsed if the widget is rendered, otherwise it the widget
		// iframe wouldn't be rendered
		if (this.rendered)
	...
	initComponent : function() {
		...
		this.on('render', function() {
			// When the widget is collapsed through the state (on page reload),
			// the height is not set. Then when expanded the portlet height is
			// incorrect, so this sets the height
