...
Ozone.marketplace.AddWidgetContainer.prototype = {
	...
	addWidget : function(config) {
		...
		if (config.data) {
			listingId = config.data.id;
			baseUrl = config.baseUrl;
		} else {
			listingId = config.widgetsJSON.itemId;
			baseUrl = config.widgetsJSON.baseUrl;
		}
		this.processMarketplaceWidgetData(baseUrl, listingId);
		return listingId;
	},
    ...
    createOwfWidgetJson: function (serviceItem, widgetId) {
        ...
        var tags = [];
        if (serviceItem.categories) {
        	for (var cat = 0; cat < serviceItem.categories.length; cat++) {
        		tags.push(serviceItem.categories[cat].title);
        	}
        }
        ...
        var widgetJson = {
            ...
            widgetVersion: serviceItem.versionName ? serviceItem.versionName : '',
