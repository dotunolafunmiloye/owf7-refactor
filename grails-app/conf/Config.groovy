...
if (BuildSettingsHolder.settings) {
	grails.config.locations = [
		...
		"file:${userHome}/.ozone/${appName}-build.groovy"
	]
} else {
	grails.config.locations = [
...
scanUsersForDuplicates.enabled = Boolean.valueOf(System.getProperty('owf.dedupUsers') ?: false)
...
// Automatically add all groups from a user's LDAP to Ozone.  Normally keep this
// false.  See SecurityFilters class for how this gets used.
autoMapLdapGroupsToOzone = Boolean.valueOf(System.getProperty('owf.autoMapLdapGroups') ?: false)
...
owf {
	...
	customHeaderFooter {
		header = '/owf/header.gsp'
		headerHeight = 20
		footer = ''
		footerHeight = 0
		jsImports = []
		cssImports = ['/owf/css/banner.css']
	}
...
environments {
	...
	development {
		...
		grails.plugin.databasemigration.updateOnStart = Boolean.valueOf(System.getProperty('owf.dbmigration.active') ?: false)
	...
	test {
		...
		// Set to false or Selenium is likely to fail.
		uiperformance.enabled = false
	}
